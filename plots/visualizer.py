...
"""
NOTE: Copied and modified from detectron2/utils/visualizer.py 
to support custom coloring and inpainting of bounding box intervals. 
For that purpose an additional class VisualizerFill is added.
"""
...
def _create_text_labels(classes, scores, class_names, draw_labels=[], is_crowd=None):
    # draw_labels is a list of labels that should be drawn
    # it contains some subset of ["class", "score"]
    labels = None

    if classes is not None and "class" in draw_labels:
        if class_names is not None and len(class_names) > 0:
            labels = [class_names[i] for i in classes]
        else:
            labels = [str(i) for i in classes]

    if scores is not None and "score" in draw_labels:
        if labels is None:
            labels = ["{:.0f}%".format(s * 100) for s in scores]
        else:
            labels = ["{} ({:.0f})%".format(l, s * 100) for l, s in zip(labels, scores)]

    # ignore "is crowd" label
    # if labels is not None and is_crowd is not None:
    #     labels = [l + ("" if crowd else "") for l, crowd in zip(labels, is_crowd)]
...
class Visualizer:
    ...
    def __init__(
        ...
        self._default_font_size = min(
            np.sqrt(self.output.height * self.output.width) // 90, 10 // scale
        )
        self._instance_mode = instance_mode
        self.keypoint_threshold = _KEYPOINT_THRESHOLD

    def draw_instance_predictions(self, predictions, draw_labels, colors, alpha=0.5):
        """
        ...
        labels = _create_text_labels(
            classes, scores, self.metadata.get("thing_classes", None), draw_labels
        )
        keypoints = (
    ...
    def draw_dataset_dict(self, dic, draw_labels, colors, alpha=0.5):
        """
...
class VisualizerFill:
    ...
    def overlay_instances(
        ...
        *,
        ...
        if areas is not None:
            ...
            # Draw fill between box PIs
            nr_x = 1000
            box_ids = torch.tensor([box_ids[idx] for idx in sorted_idxs])
            for box_id in box_ids.unique()[box_ids.unique() != 0]:
                a, b = torch.nonzero(box_ids == box_id, as_tuple=True)[0]
                if areas[a] >= areas[b]:
                    pi_l = b
                    pi_u = a
                else:
                ...
                x0_u, y0_u, x1_u, y1_u = boxes[pi_u]

                x0_l, y0_l, x1_l, y1_l = boxes[pi_l]

                self.output.ax.fill_between(
                    x=np.linspace(x0_l, x1_l, nr_x),
                    y1=y0_l,
                    y2=y0_u,
                    color=fill_color,
                    alpha=alpha[2],
        ...
        for i in range(num_instances):
            color = assigned_colors[i]
            if boxes is not None:
                alph = alpha[0] if box_ids[i] == 0 else alpha[1]
                self.draw_box(boxes[i], edge_color=color, alpha=alph, lw=lw)

            if masks is not None:
                for segment in masks[i].polygons:
                    self.draw_polygon(segment.reshape(-1, 2), color, alpha=alpha[0])
    ...
    def draw_box(self, box_coord, alpha=0.5, edge_color="g", line_style="-", lw=None):
        """
        Args:
            box_coord (tuple): a tuple containing x0, y0, x1, y1 coordinates, where x0 and y0
                are the coordinates of the image's top left corner. x1 and y1 are the
        ...
        linewidth = lw if lw is not None else max(self._default_font_size / 4, 1)
