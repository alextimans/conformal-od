...
class YOLOStdConformal(RiskControl):
    """
    ...
    def set_collector(self, nr_class: int, nr_img: int, dict_fields: list = []):
        self.collector = YOLOStdConformalDataCollector(
            nr_class,
            nr_img,
            dict_fields,
            self.logger,
    ...
    def evaluate(
        ...
        ), "YOLOStdConformal is currently not evaluted for label sets."
...
class YOLOStdConformalDataCollector(DataCollector):
    def __init__(
        self,
        nr_class: int,
        nr_img: int,
